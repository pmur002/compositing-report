...
TARFILE = ../compositing-deposit-$(shell date +'%Y-%m-%d').tar.gz
...
docker:
	...
	sudo docker run -e Rscript=Rscript -v $(shell pwd):/home/work/ -w /home/work --rm pmur002/compositing-report make compositing.html

web:
	make docker
	cp -r ../compositing-report/* ~/Web/Reports/GraphicsEngine/compositing/
